...
首先，我们将在 Jupyter Lab 中打开一个笔记本 (`hello.ipynb`)，并将其渲染为几种格式。
如果你想在自己的环境中逐步学习，请下载下面的笔记本。

::: {.callout-note appearance="minimal"}
<i class="bi bi-journal-code"></i> [Download hello.ipynb](_hello.ipynb){download="hello.ipynb"}
:::

然后，创建一个新的工作目录，将笔记本复制到该目录，并在终端中切换到该目录。

接下来，执行以下命令安装 JupyterLab 以及教程中使用的软件包（ `matplotlib` 和 `plotly` ），并打开教程笔记本：
...
使用 Quarto 渲染 Jupyter notebook时，笔记本的内容（代码、markdown 和输出）会被转换为纯 markdown，然后由 [Pandoc](http://pandoc.org/)处理，生成最终格式。
...
我们的笔记本中有几种不同类型的单元格，让我们对每种类型的单元格进行一下处理。
...
请尝试更改 `fig-cap` 和/或代码，运行单元格，然后保存笔记本，查看更新后的预览效果。
